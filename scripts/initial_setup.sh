...
do
    ...
    SHIP_PLANS_JSON=$(echo "$SHIP_PLANS_JSON" | jq --arg nm "$CLUSTER_NAME" --arg dsh "-" --arg reg "$CLUSTER_REGION" --arg dk "$DECK_SUBDOMAIN" '. as $parent | .ship_plans[($nm + $dsh + $reg)] | . * {deckSubdomain: $dk} | $parent')
    echo "NEW SHIP_PLANS_JSON : "$(echo $SHIP_PLANS_JSON | jq '.')""
    echo "-----------------------------------------------------------------------------"
    ...
    SHIP_PLANS_JSON=$(echo "$SHIP_PLANS_JSON" | jq --arg nm "$CLUSTER_NAME" --arg dsh "-" --arg reg "$CLUSTER_REGION" --arg gt "$GATE_SUBDOMAIN" '. as $parent | .ship_plans[($nm + $dsh + $reg)] | . * {gateSubdomain: $gt} | $parent')
    echo "NEW SHIP_PLANS_JSON : "$(echo $SHIP_PLANS_JSON | jq '.')""
    echo "-----------------------------------------------------------------------------"
    ...
    SHIP_PLANS_JSON=$(echo "$SHIP_PLANS_JSON" | jq --arg nm "$CLUSTER_NAME" --arg dsh "-" --arg reg "$CLUSTER_REGION" --arg x509 "$X509_SUBDOMAIN" '. as $parent | .ship_plans[($nm + $dsh + $reg)] | . * {x509Subdomain: $x509} | $parent')
    echo "NEW SHIP_PLANS_JSON : "$(echo $SHIP_PLANS_JSON | jq '.')""
    echo "-----------------------------------------------------------------------------"
    ...
    SHIP_PLANS_JSON=$(echo "$SHIP_PLANS_JSON" | jq --arg nm "$CLUSTER_NAME" --arg dsh "-" --arg reg "$CLUSTER_REGION" --arg vlt "$VAULT_SUBDOMAIN" --arg wd "$DOMAIN_TO_MANAGE" '. as $parent | .ship_plans[($nm + $dsh + $reg)] | . * {vaultSubdomain: $vlt, wildcardDomain: $wd} | $parent')
    echo "NEW SHIP_PLANS_JSON : "$(echo $SHIP_PLANS_JSON | jq '.')""
    n=$((n+1))
done
